...
function MainApp() {
  ...
  const startLongPress = useCallback(() => {
    setPressing(true); setPressProgress(0);
    const startTime = Date.now();
    pressAnimFrame.current = setInterval(() => {
      setPressProgress(Math.min((Date.now() - startTime) / 1000, 1));
    }, 16);
    longPressTimer.current = setTimeout(() => {
      dispatch({ type: 'NEW_REPORT' }); setPressing(false); setPressProgress(0);
  ...
  const cancelLongPress = useCallback(() => {
    ...
    setPressing(false); setPressProgress(0);
  }, []);
  ...
  const handleAddExpense = () => {
    ...
    if (val > 0 && cat) {
      ...
      setExpCategory(''); setExpAmount(''); setExpHasReceipt(false); setExpCustomCategory('');
    }
  };

  const selectCategory = (label: string) => { setExpCategory(label); setExpCustomCategory(''); };
  ...
  if (screen === 'report-preview') {
    ...
    return <ReportPreview transactions={state.transactions} totalTopup={totalTopup} totalExpense={totalExpense} finalBalance={state.balance} startDate={dates[dates.length - 1] || new Date().toISOString()} endDate={dates[0] || new Date().toISOString()} onBack={() => setScreen('main')} />;
  }
  if (screen === 'reports-history') return <ReportsHistory reports={state.reports} currentTransactions={state.transactions} currentBalance={state.balance} onBack={() => setScreen('main')} />;
      ...
      <div className="sticky top-0 z-10 bg-[#f2f2f7]/70 backdrop-blur-2xl border-b border-white/60">
        <div className="max-w-lg mx-auto flex items-center justify-between px-4 py-2">
          ...
          <div className="flex items-center gap-0.5">
            <button onClick={() => setScreen('reports-history')} className="p-2.5 rounded-full active:bg-black/5 transition"><FileText size={20} className="text-[#007aff]" /></button>
            <button onClick={() => setScreen('settings')} className="p-2.5 rounded-full active:bg-black/5 transition"><SettingsIcon size={20} className="text-[#007aff]" /></button>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-lg mx-auto px-4 pt-4 pb-8">
        ...
        <div className="relative overflow-hidden rounded-[20px] bg-gradient-to-br from-[#ff2d55] to-[#ff6b6b] p-5 mb-4 shadow-lg shadow-red-300/30">
          ...
          <div className="relative">
            ...
            <p className="text-[32px] font-extrabold text-white tracking-tight leading-tight">{formatMoney(state.balance)}</p>
            {state.balance < 0 && <div className="inline-flex mt-1.5 bg-white/20 backdrop-blur-md px-2.5 py-1 rounded-full"><span className="text-[11px] font-semibold text-white">⚠️ Долг: {formatMoney(Math.abs(state.balance))}</span></div>}
            {state.transactions.length > 0 && (
              <div className="flex gap-5 mt-4 pt-3 border-t border-white/20">
                <div className="flex items-center gap-1.5"><TrendingUp size={12} className="text-green-300" /><span className="text-[11px] text-white/70 font-medium">+{formatMoney(totalIn)}</span></div>
                <div className="flex items-center gap-1.5"><TrendingDown size={12} className="text-white/50" /><span className="text-[11px] text-white/70 font-medium">−{formatMoney(totalOut)}</span></div>
              </div>
            )}
            <button onClick={() => setShowTopup(true)} className="mt-4 flex items-center justify-center gap-2 w-full bg-white/20 hover:bg-white/30 backdrop-blur-md text-white py-2.5 rounded-2xl font-semibold text-sm transition active:scale-[0.97] border border-white/10">
              <Plus size={16} strokeWidth={2.5} /><span>Пополнить баланс</span>
            </button>
          </div>
        </div>
        ...
        <div className="bg-white/70 backdrop-blur-xl rounded-[20px] border border-white/80 shadow-sm mb-4 overflow-hidden">
          <div className="px-4 pt-4 pb-1.5"><span className="font-semibold text-[#1c1c1e] text-[15px]">Новый расход</span></div>
          <div className="px-4 pb-4 space-y-3 pt-1">
            <div className="grid grid-cols-3 gap-1.5">
              {state.quickCategories.map((label: string) => (
                <button key={label} onClick={() => selectCategory(label)} className={`px-2 py-2 rounded-2xl text-[13px] font-medium transition-all ${expCategory === label && !expCustomCategory.trim() ? 'bg-[#ff2d55] text-white shadow-sm shadow-red-200/60' : 'bg-[#e5e5ea]/60 text-[#1c1c1e] active:scale-95 active:bg-[#d1d1d6]'}`}>{label}</button>
              ))}
            </div>
            {!expCategory && <input type="text" placeholder="Или своя категория..." value={expCustomCategory} onChange={e => setExpCustomCategory(e.target.value)} className="w-full rounded-2xl px-4 py-2.5 text-sm bg-[#e5e5ea]/50 placeholder:text-[#8e8e93] text-[#1c1c1e] focus:outline-none focus:ring-2 focus:ring-[#ff2d55]/30 border border-white/50 transition" />}
            {expCategory && (
        ...
        {state.transactions.length > 0 && (
          <button onClick={() => setScreen('report-preview')} className="w-full flex items-center justify-center gap-2 bg-white/70 backdrop-blur-xl text-[#007aff] py-2.5 rounded-2xl font-medium text-sm border border-white/80 shadow-sm mb-4 active:scale-[0.98] transition">
            <FileText size={15} /><span>Предварительный просмотр отчёта</span>
          </button>
        )}

        {/* History */}
        <div className="mb-4">
          <h2 className="text-[11px] font-semibold text-[#8e8e93] uppercase tracking-wider mb-2 px-1">История операций</h2>
          {state.transactions.length === 0 ? (
        ...
        {state.transactions.length > 0 && (
          <div className="mt-6 mb-4">
            <button onMouseDown={startLongPress} onMouseUp={cancelLongPress} onMouseLeave={cancelLongPress} onTouchStart={startLongPress} onTouchEnd={cancelLongPress} onTouchCancel={cancelLongPress} className="w-full py-3 rounded-2xl text-[13px] font-medium text-[#8e8e93] bg-[#e5e5ea]/40 backdrop-blur-sm border border-white/60 select-none relative overflow-hidden transition">
              {pressing && <div className="absolute inset-0 bg-[#ff2d55]/15" style={{ width: `${pressProgress * 100}%` }} />}
              <span className="relative z-10">{pressing ? 'Удерживайте...' : 'Зажмите для нового отчёта'}</span>
            </button>
            <p className="text-center text-[10px] text-[#c7c7cc] mt-1.5">Удерживайте 1 сек для завершения периода</p>
          </div>
        )}
      </div>

      {showTopup && <TopupModal onClose={() => setShowTopup(false)} onTopup={(amount) => dispatch({ type: 'TOPUP', amount })} />}
      {editingTransaction && <EditTransactionModal transaction={editingTransaction} onClose={() => setEditingTransaction(null)} onSave={(id, category, amount, hasReceipt) => dispatch({ type: 'EDIT_TRANSACTION', id, category, amount, hasReceipt })} onDelete={(id) => dispatch({ type: 'DELETE_TRANSACTION', id })} />}
    </div>
  );
}

export default function App() {
  return <ErrorBoundary><StoreProvider><MainApp /></StoreProvider></ErrorBoundary>;
}
